...
- **ONE PRICER (PP-0, FLIPPED 2026-07-16).** `@ilaunchify/plans` `computeOrderPricing` is the ONLY thing that prices an order, and `placeOrder` charges its output. The PDP, configurator, checkout estimate and sample flow all call it; `resolveGoods` + `composeProductionLines` declare the basis (pack orders price on the agreed PACK_PRICE, legacy non-pack on the catalog buildup) and compose each add-on exactly once. This retired `Math.max(costBuildup, dispatchSubtotal, packPrice)`, which maxed two COSTS against one PRICE. **Cost and price must never share an expression:** the "never fund below partner cost" kernel survives as `costFloorBreach`, which REPORTS to ops and adds nothing to a bill. `pnpm pp0:delta` shows what the flip changed. Client components import `@ilaunchify/plans/math` (the barrel re-exports server-only `lookups`, which imports prisma).
- **RETIRED:** the flat 5% `OrderSettings.productionFeeBps` **and** `OrderSettings.samplePlatformFeeBps` as fee sources (columns kept, deprecated). **A sample is not a different KIND of order, just a small one** (Pavel 2026-07-16): it carries the creator's tier rate like everything else. **Credit-back is retired** too: a sample is paid in full at order time and mints nothing (`getSampleSettings` forces `creditBackEnabled` false, because the DB row wins a default flip). A `check:invariants` rule warns on any new hardcoded platform-fee constant outside `@ilaunchify/plans`.
- **NO MERIT ON NON-PRODUCT LEGS (CONFIRMED 2026-07-16, closes `FEE_MODEL_RECONCILIATION_SPEC` §6.4, open since 07-09).** `routing.ts:858` is load-bearing, **do NOT "fix" it.** Merit prices a CHOICE, and the manufacturer is the only leg a creator chooses and pins. See `SERVICE_SYMMETRY_AND_MERIT_2026-07-15.md`.
...
- `SERVICE_SYMMETRY_AND_MERIT_2026-07-15.md` (NEW, **read before touching merit**): **THE INSTRUMENT MUST MATCH THE SELECTION MODEL.** Merit judges ONLY the manufacturing/PRODUCT leg, because that is the only leg the creator CHOOSES and PINS; a printer is rotated on rating, an FC is selected on fit, a co-pack leg is auto-derived. So: no merit on non-PRODUCT legs (CONFIRMED, closes FEE_MODEL_RECONCILIATION §6.4, the last open flag); `routing.ts:858` is load-bearing, do NOT "fix" it; merit moves `Partner.tier` (org) -> `PartnerService.meritTier` (MANUFACTURING only, nullable, null = NOT JUDGED, never a default VERIFIED). Ratings != merit: ratings are service-scoped + universal (they drive rotation + FC scoring); merit is manufacturing-only (badge + fee). Also §4 = the full 4-service picture + `FC_MONETIZATION_GAP_2026-07-15.md`.
