  ...
  return (
    <div className="mx-auto max-w-[1080px] pb-24">
      ...
      <div className="flex items-center gap-[5px] overflow-x-auto rounded-t-2xl border border-ink-200 bg-ink-50 px-5 py-[11px]">
            ...
            <div key={label} className="flex items-center gap-[5px]">
              <button
                ...
                className={`flex items-center gap-2 whitespace-nowrap rounded-pill border px-[13px] py-[7px] text-[12.5px] font-semibold transition ${
                  state === 'on'
                    ? 'border-pink-200 bg-white text-ink-900 shadow-sm'
                    : state === 'done'
                      ? 'border-transparent text-success-700'
                      : 'border-transparent text-ink-500'
                }`}
              >
                <span
                  className={`grid h-5 w-5 flex-none place-items-center rounded-full text-[11px] font-extrabold ${
                    state === 'on' ? 'bg-pink-500 text-white' : state === 'done' ? 'bg-success-500 text-white' : 'bg-ink-200 text-ink-600'
                  }`}
                >
                  {i < v ? '✓' : i + 1}
                </span>
        ...
        <button
          type="button"
          onClick={() => setV((x) => Math.min(STAGES.length - 1, x + 1))}
          disabled={v >= STAGES.length - 1}
          className="rounded-pill bg-ink-900 px-4 py-[9px] text-[12.5px] font-bold text-white hover:bg-black disabled:cursor-not-allowed disabled:opacity-40"
        >
          Next stage →
        </button>
      </div>
