...
/** A confirmed parse line ready to seed into the live recipe. */
export interface CommittedParseLine {
  ingredientId: string
  weightG: number
  name: string
}
...
interface AiParserPanelProps {
  ...
  /** Receives the accepted lines so the builder can seed + autosave them. */
  onCommit: (lines: CommittedParseLine[]) => void
  onCancel: () => void
}
...
export function AiParserPanel({ productTemplateId, onCommit, onCancel }: AiParserPanelProps) {
  ...
  function handleCommit() {
    ...
    startCommit(async () => {
      ...
      const committed = res.committed ?? committable.length
