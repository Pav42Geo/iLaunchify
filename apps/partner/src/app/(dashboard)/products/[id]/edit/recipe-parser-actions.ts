...
import { addIngredientSlot, setRecipeEntryMode } from './card-actions'
...
// -----------------------------------------------------------------------------
// commitParsedSlots — write accepted lines through addIngredientSlot.
// -----------------------------------------------------------------------------
...
export async function commitParsedSlots(
  ...
  ok: boolean
  error?: ParseError
  results?: Array<{ lineNumber: number; ok: boolean; error?: string }>
}> {
  const { user, partner, error } = await authorizeParser(productTemplateId)
  if (error) return { ok: false, error }

  // Record AI_PARSER as the primary method BEFORE the slot loop. addIngredientSlot
  // stamps SEARCH_BUILD when recipeEntryMode is null (Slice 2), so stamping here
  // first ensures an AI-built recipe is recorded as AI_PARSER, not SEARCH_BUILD.
  // (Deviates from the brief, which placed this after the loop — that order
  // would mis-record the method. See decision log.)
  await setRecipeEntryMode(productTemplateId, 'AI_PARSER')

  const results: Array<{ lineNumber: number; ok: boolean; error?: string }> = []
  for (const line of acceptedLines) {
    const res = await addIngredientSlot({
      productTemplateId,
