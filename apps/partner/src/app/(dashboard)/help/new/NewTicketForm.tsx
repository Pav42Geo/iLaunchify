...
import { Search, Boxes, Package, Check, AlertCircle, ArrowRight } from 'lucide-react'
...
export function NewTicketForm({
  ...
  // Deflection: when a dispatch/product is attached, surface existing open tickets
  // on it so the partner reuses the thread instead of filing a duplicate.
  useEffect(() => {
    ...
    let cancelled = false
    getEntityOpenTickets({ entityType: KIND_TO_ENTITY[attachType], entityId })
      .then((rows) => {
...
// Deflection panel — existing open tickets on the attached dispatch/product.
