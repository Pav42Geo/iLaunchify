// Partner → Help → ticket detail. Partner-scoped: getTicket throws if the ticket
...
export default async function PartnerTicketDetailPage({ params }: PageProps) {
  const { ticketId } = await params
  const user = await requireUser()

  let ticket
  try {
    ticket = await getTicket(ticketId, { role: 'PARTNER', userId: user.id })
  } catch (err) {
