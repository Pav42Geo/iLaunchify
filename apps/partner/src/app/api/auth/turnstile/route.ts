...
//
// Magic-link / Google sign-in run through Auth.js signIn() client-side, so we verify
// the Turnstile token BEFORE that, keeping the Auth.js flow untouched. The login form
// submits { token } here first and only calls signIn(...) on { ok: true }.
//
