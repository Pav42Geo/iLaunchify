...
// =============================================================================
// Partner Die-line Studio shell — mirrors the creator Design Studio chrome with
// partner-only tools. Parallel shell (NOT a fork of the creator's), reusing the
// shared frame model from @ilaunchify/ui. docs/DIELINE_FRAME_EDITOR_SPEC.md §7b.
//
//   Top bar · Left tool rail (Die-line·Surfaces·Guides·Frames·Layers) ·
//   Drawer · Center canvas (uploaded backdrop + draggable boxes/frames) ·
//   Bottom zoom toolbar.
// =============================================================================
...
// Shared presentation layer — single source of truth, also used by the inline
// PackagingStudioStep (docs/HANDOFF-TO-CODE-dieline-studio-dedupe.md).
...
export function DielineStudioShell({ dieline }: { dieline: DielineEditorData }) {
  ...
  const [layout, setLayout] = useState<FrameLayout>(
    () => (dieline.frames as FrameLayout) ?? structuredClone(DEFAULT_FRAME_LAYOUT),
  )
  const [trim, setTrim] = useState<NormBox>(() => asBox(dieline.trimBox, { x: 0, y: 0, w: 1, h: 1 }))
  const [safe, setSafe] = useState<NormBox>(() => asBox(dieline.safeAreaBox, { x: 0.05, y: 0.05, w: 0.9, h: 0.9 }))
  const [showGuides, setShowGuides] = useState({ trim: true, safe: true })
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [zoom, setZoom] = useState(1)
  const [saveStatus, setSaveStatus] = useState<'idle' | 'saving' | 'saved'>('saved')
  const [confirmed, setConfirmed] = useState(dieline.status === 'PARTNER_CONFIRMED' || dieline.status === 'ADMIN_VERIFIED' || dieline.status === 'ACTIVE')
  ...
  const queueSave = useCallback((nextLayout: FrameLayout, nextTrim: NormBox, nextSafe: NormBox) => {
    ...
    saveTimer.current = setTimeout(async () => {
      const [a, b] = await Promise.all([
        saveDielineFrames(dieline.id, nextLayout),
        saveDielineGeometry(dieline.id, { trimBox: nextTrim, safeAreaBox: nextSafe }),
      ])
    ...
    <div className="flex h-full w-full flex-col bg-zinc-100 text-ink-900">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="w-[340px] shrink-0 overflow-y-auto border-r border-ink-200 bg-white">
          {tool === 'dieline' && <DielineDrawer dieline={dieline} />}
        ...
        <main className="relative flex min-w-0 flex-1 items-center justify-center overflow-auto bg-[radial-gradient(circle,#e4e4e7_1px,transparent_1px)] bg-[length:18px_18px] p-8">
          <div style={{ transform: `scale(${zoom})` }} className="transition-transform">
            ...
            >
              {/* backdrop */}
              {dieline.fileUrl && dieline.originalFileFormat === 'PDF' && (
                <embed src={dieline.fileUrl} type="application/pdf" className="pointer-events-none absolute inset-0 h-full w-full opacity-90" />
              )}
              {dieline.fileUrl && dieline.originalFileFormat !== 'PDF' && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={dieline.fileUrl} alt="die-line" className="pointer-events-none absolute inset-0 h-full w-full object-contain opacity-90" />
              )}
              {!dieline.fileUrl && (
...
function DielineDrawer({ dieline }: { dieline: DielineEditorData }) {
  return (
    <div>
      <DrawerHead title="Die-line" sub="The uploaded prepress file behind your boxes." />
      <div className="space-y-2 px-4 py-3 text-[12.5px]">
        <Row label="File">{dieline.fileUrl ? (dieline.originalFileFormat ?? 'file') : '— none —'}</Row>
        <Row label="Width">{dieline.widthMm != null ? `${dieline.widthMm} mm` : '—'}</Row>
        <Row label="Height">{dieline.heightMm != null ? `${dieline.heightMm} mm` : '—'}</Row>
        <Row label="Bleed">{dieline.bleedMm} mm</Row>
