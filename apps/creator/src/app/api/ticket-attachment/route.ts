...
//
// Creator-scoped download for a ticket reply attachment. getTicket throws if the
// ticket isn't the creator's, and the key must belong to a reply on that ticket —
...
export async function GET(req: NextRequest) {
  ...
  try {
    const ticket = await getTicket(ticketId, { role: 'CREATOR', userId: user.id })
