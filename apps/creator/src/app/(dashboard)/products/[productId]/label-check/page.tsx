      ...
      {!res.hasDieline || !res.report ? (
        ...
        <>
          ...
          >
            ...
            <div>
              <p className={`font-display text-[15px] font-semibold ${res.report.status === 'pass' ? 'text-emerald-900' : 'text-rose-900'}`}>
            ...
            {res.report.checks.map((c) => (
              <li
                ...
                className={`flex items-start gap-2.5 rounded-xl border px-4 py-2.5 text-[12.5px] ${
                  c.status === 'pass' ? 'border-ink-100 bg-white' : 'border-rose-200 bg-rose-50/50'
                }`}
              >
                {c.status === 'pass' ? (
                ...
                <div>
                  ...
                  {c.issues.map((iss, i) => (
                    <p key={i} className="text-[11.5px] text-rose-700">{iss.message}</p>
                  ))}
                </div>
              </li>
            ))}
