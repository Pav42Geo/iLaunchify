// Admin — Partner Access mode toggle (private invite-only ↔ public signup).
// docs/PARTNER_ONBOARDING_STRATEGY_2026-07.md §7. Server-rendered: reads the
// mode via getPartnerAccessMode() and flips it through the audited
// setPartnerAccessMode() action (bound form buttons — no client component).

import { requireCapability } from '@ilaunchify/auth'
import { getPartnerAccessMode } from '@ilaunchify/db'
...
export default async function PartnerAccessPage() {
  ...
  const mode = await getPartnerAccessMode()
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        description="Control whether the partner network is invite-only (private) or open to public signup. This drives the “Become a partner” vs “Sign up” calls-to-action and gates onboarding entry."
      />
      ...
      <div className="rounded-2xl border border-ink-200 bg-white p-5">
        ...
        <p className="mt-3 max-w-2xl text-[13px] text-ink-600">
          {isPrivate
            ? 'New partners can only join by admin invitation. The public form collects a Lead (in /admin/leads); nobody self-provisions an account. CTAs read “Become a partner”.'
