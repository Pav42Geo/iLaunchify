...
// C7 — variant manager for a LabelingSymbol (label + reproduction standards +
// SVG/PNG assets). Mirrors the packaging-variant manager.
...
import {
  ...
  deleteLabelingSymbolVariant,
  uploadLabelingVariantSvg,
  uploadLabelingVariantPng,
} from './actions'

export interface LblVariantView {
  id: string
  label: string
    ...
    <div className="space-y-4">
      ...
      <ul className="space-y-3">
        {variants.map((v) =>
          editingId === v.id ? (
            <li key={v.id}>
              <VariantForm labelingSymbolId={labelingSymbolId} initial={v} onClose={() => setEditingId(null)} />
            </li>
          ) : (
            <li key={v.id}>
...
function VariantRow({ variant: v, onEdit }: { variant: LblVariantView; onEdit: () => void }) {
  ...
  function remove() {
    ...
    startTransition(async () => {
      const res = await deleteLabelingSymbolVariant(v.id)
      if (!res.ok) {
  ...
  function upload(file: File, kind: 'SVG' | 'PNG') {
    startTransition(async () => {
      ...
      fd.set('file', file)
      const res = kind === 'SVG' ? await uploadLabelingVariantSvg(fd) : await uploadLabelingVariantPng(fd)
      if (!res.ok) {
        toast.error(res.error)
        return
      }
...
function VariantForm({
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = initial
        ? await updateLabelingSymbolVariant({ ...payload, id: initial.id })
        : await createLabelingSymbolVariant(payload)
      if (!res.ok) {
        setError(res.error)
        return
      }
