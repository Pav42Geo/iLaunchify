...
- **Pause timeline (Pavel 2026-07-20/21, FINAL; supersedes the same-day keep-benefits variant):** benefits are kept until the END of the already-paid period (`tierPauseStartsAt` = `current_period_end`; they paid for it), then the tier DROPS TO MAKER for the 1 to 3 unpaid months, then billing resumes and the remembered tier (`tierPausedFromTier`) restores automatically. Rationale for the drop: not paying = no paid benefits, and specifically no tier-gated AI features whose inference the platform pays for (Pavel). Mechanics: Stripe `pause_collection` behavior `void` anchored to period end; the tier-dunning cron runs `processTierPauseStarts` to sweep the drop (audited `CREATOR_TIER_CHANGE`, label `pause_started`, designer seats auto-swept); the `customer.subscription.updated` webhook restores on resume (`pause_resumed`); `unpauseTierSubscription` handles manual early resume (`pause_early_resume`, cooldown stands). Guards: pause offered only for NOT_USING / TEMPORARY reasons, one pause per rolling 365 days (`tierLastPausedAt`), pause withdraws a pending cancel, upgrades blocked while a pause is in flight.
