...
- **REUSES the existing `PartnerFacility` model** (do NOT create a second facility model — that was a mistake, caught on push). `PartnerFacility` already has `partnerId, name, addressLine1/2, city, region (=state), postalCode, country, isDefault` and is used by product defaults + variants. Added `lat?`/`lng?` (Phase 3 proximity) + back-relations `services`, `certificateInstances`. `isDefault` = the primary facility.
- **Nullable `facilityId`** + `facility PartnerFacility?` relation + index on `PartnerService` and `PartnerCertificateInstance`. ✅
- **Backfill script** `packages/db/prisma/backfill-facilities.ts` (`pnpm --filter @ilaunchify/db backfill:facilities`) — links facility-less services + certs to each partner's existing `isDefault` PartnerFacility; if none exists AND the partner has a full address, creates one (`PartnerFacility` requires addressLine1/city/region/postalCode, so address-less partners are skipped and keep `facilityId=null`). Idempotent. Run after `db:push`+`db:generate`.
