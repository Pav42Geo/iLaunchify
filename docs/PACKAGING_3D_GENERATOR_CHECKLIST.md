...
- [ ] G3.1c **(NEXT, visible)** render the lathe container in `Packaging3DView` via `THREE.LatheGeometry(profile)`; wrap the label die-line onto the body band (cylindrical UV at `2πr × h`). Needs a `dielineClass` (STRUCTURAL|LABEL) + ContainerModel spec on PackagingType (schema — propose).
- [ ] G3.1d **AI image-to-3D admin-assist (fenced, later)** — irregular containers only; Meshy/Rodin/Tripo API or self-host TripoSR; admin-verified + spec-scaled; never the primary/label-precise path (plan §2.5).
