...
- [x] G1.4 glTF design-texture swap — `Dieline3DViewer` takes a `modelUrl` prop (jsm GLTFLoader): loads the imported model, centers/scales, applies the design texture to its materials, env-lit; gated on `modelUrl` (existing callers unaffected). **Plumbed end-to-end**: design page `loadPackagingModel3dUrl` (signs `PackagingType.model3dKey`) → `CanvasLayoutShell` → `LivePreview3DDock` → viewer. A product whose packaging type has an uploaded glTF now shows the REAL model with the design in the Live 3D preview. Typecheck-green; needs browser QA. **Per-surface binding added** — `materialTakesDesign(name, target)` applies the design only to the imported model's matching material (label→body materials, lid sticker→cap materials; safe fallback to all if no names match), mirroring the `gltf-surface-binding` keyword vocabulary. **Exact binding done** — the packaging type's authored `defaultSurfaces` are plumbed (page `loadPackagingModel3d` → shell → dock → viewer `modelSurfaces`); the viewer runs `bindGltfMaterialsToSurfaces(materialNames, surfaces)` to bind material→surface by the admin's map, targets the surface matching the design's face (top=lid, else front/body), and applies the design there — falling back to the name heuristic, then all, so it never renders blank.
- [~] G1.4b **Smart preview** — `previewIntentForCategory(dieCutCategory)` drives which surface carries the design + opening camera: lid/top stickers (`LID_LABEL`/`TUB_LID`) → design on the lid cap, opened top-down; body wraps → body, front-on. Wired dock → viewer (`designSurface`/`initialView`). Removed the meaningless random-shape switcher + descriptive text. **QA: confirm top-view camera sign + add any other lid/top category names.**
