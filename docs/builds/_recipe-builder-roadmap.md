...
- **2026-06-01 (Slice 4) — GATE: free for all partners (Pavel).** `declare_nutrition_panel` boolean = true on all 3 partner plans (admin-tunable). Rationale: COA holders are sometimes small-batch — don't wall out the persona who needs Mode 3 most. Gated via the same partner-plan-feature pattern as Mode 2, just all-true (DECLARED_PANEL tile enabled for everyone).
- **2026-06-01 (Slice 4)** — `ProductTemplate` lacked servingSize/servingsPerContainer/ingredientStatement/netQuantity. Instead of 4 typed columns, stored the whole declaration as one `declaredPanel Json?` (the typed `PanelData` + ingredientStatement + netQuantity + allergens). Plus `nutrientSource` enum (COMPUTED default / DECLARED) + `Ingredient.isDeclaredPanelSynthetic`. The per-serving panel is stored faithfully and renders directly — no Python round-trip needed for the declared values.
- **2026-06-01 (Slice 4) — TWO FOLLOW-UPS (cross-service / fixture):** (a) The real nutrition summation is the **Python compliance service** (`services/compliance`, not in this repo's TS) — its `nutrientSource === 'DECLARED'` branch (return the synthetic ingredient's panel + declaredByManufacturer) is Pavel/Python work, NOT done here. (b) Marketing's nutrition panel is **fixture-driven** (`detail.nutrition` from `findTemplateDetail`), so the actual declared values don't render there yet — but the **disclosure banner ships now**, keyed on the real DB `nutrientSource` (via `getProductNutrientSource(slug)`). The renderer caption (`declaredByManufacturer`) + partner live-preview are fully wired.
- **2026-06-01 (Slice 4)** — Synthetic "Whole Product" ingredient: `findFirst` by name `__declared_panel__<templateId>` + ownerPartnerId + isDeclaredPanelSynthetic → update/create (no unique constraint needed). Real Ingredient fields are `source`/`ownerPartnerId` (brief's `sourceType`/`partnerServiceId` don't exist). declareNutritionPanel OVERWRITES recipeEntryMode to DECLARED_PANEL (declaring replaces the whole recipe — unlike incremental first-writer-wins). Reseed via `seed:subscription-plans`, restart next dev.
- **2026-06-01 (Slice 4)** — DeclaredPanelPanel V1: form starts fresh (no prefill-on-edit — follow-up); Supplement Facts reuses the Nutrition grid (Supplement-specific rows V1.1); %DV is manual/optional (auto-DV is a follow-up).
