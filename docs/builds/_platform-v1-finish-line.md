...
- **2026-06-03 — P8 VERIFIED GREEN.** Ran the drain/verify sequence: cockroach up, `pnpm install`, `prisma migrate deploy` → `migrate status` = "up to date", `prisma db seed` clean, `seed:subscription-plans` (15 features/partner plan), **full workspace typecheck 18/18**. No pending migrations; the backlog the punch list listed is already drained.
- **2026-06-03 — P7 FOUND + FIXED a launch-blocking env-name bug.** The code reads `process.env.R2_PUBLIC_BASE_URL` (Design Studio image uploads in `canvas/actions.ts` + cert PNG badge public URLs in `certificate-types/actions.ts`), but `.env.example` declared `R2_PUBLIC_URL` — which **nothing in the code reads**. Setting the prod env per the example would silently break studio uploads + marketplace cert-badge images. Fixed `.env.example` → `R2_PUBLIC_BASE_URL` (with doc) and made both code sites accept either name (`R2_PUBLIC_BASE_URL ?? R2_PUBLIC_URL`).
- **2026-06-03 — P5 code-verified (runtime = Pavel).** Both webhook routes (creator + partner) verify the Stripe signature via `constructEvent` (400 on missing/bad sig) and `handleStripeEvent` covers all V1 events: `payment_intent.succeeded` (Order→PAID + dispatches — this is the order-spawn event, NOT checkout.session.completed which is tier onboarding), `invoice.payment_succeeded`, `checkout.session.completed`, `customer.subscription.updated/deleted`, `charge.refunded`, `transfer.created/updated/reversed`. The code path is sound; firing events with `stripe trigger` needs Pavel's CLI + test keys.
