...
- 2026-07-02 (Cowork, slice 11 — P3 opens, D1): **multi-seat team foundation** per PRINT_PRODUCTION_WORKFLOW §2.1–2.7. Schema: `PartnerMembership` (org-wide, isAdmin, soft-remove) + `PartnerServiceMembership` (service-scoped PREPRESS/PRODUCTION roles) + `PartnerInvite` (token, 14d TTL, grant payload) + 2 enums. **Deviation from spec draft: `Partner.userId` KEEPS @unique** as founder-of-record — teammates join via memberships; dropping the constraint would break every findUnique guard for zero gain. New `getPartnerAccess(userId)` / `requirePartnerAdminAccess` in @ilaunchify/auth — THE guard for partner surfaces (founder path lazily backfills an isAdmin membership; teammate path resolves service scoping); existing founder-only guards stay correct, migrate surface-by-surface. UI: `/settings/team` (members, pending invites w/ revoke, invite form w/ admin flag + per-service role checkboxes; ACTIVE-gated, admin-only) + `/invite/[token]` accept page (outside dashboard group; email-match enforced; consuming a valid invite flips role→PARTNER; V1 one-org-per-user). Invite email via sendTransactionalEmail. NOT yet done (next P3 slices): guard migration sweep to getPartnerAccess, role-scoped nav/queues (prepress vs production), role-routed notifications (§6.3), lastActiveAt stamping. **db:push + generate needed.** Caveat: logged-out invitees relying on magic-link auto-create — verify the Auth.js email provider creates users for unknown addresses; if not, invitees sign up first, then reopen the link.
- 2026-07-02 (Cowork, slice 12 — guard migration): **teammates are functional.** New `serviceOwnedBy(userId)` fragment (`apps/partner/src/lib/partner-context.ts`) — org-admin path OR service-scoped-membership path, deny-by-default — replaced founder-only `partner:{userId}` across the operational core: dispatch layout (now `getActingPartner`, resolves founders AND teammates; nav scoped to workable services), dashboard, orders inbox, dispatch detail + all FSM/proof/lot/label/ship-doc/release actions, manifest route, inbound/inventory/outbound data + guards. Commercial surfaces admin-gated via `requirePartnerAdminAccess`: /billing, /settings/fulfillment (+ all its actions), and the non-admin nav hides Products/Packaging/Payments/etc (roleNavFor isOrgAdmin param). Notification prefs stay per-member (FC events scoped to workable services). Founder-only by DESIGN: onboarding, /help (personal tickets); /products+/packaging internals still founder-guarded (nav-hidden for members; migrate when service-scoped catalog editing is actually wanted). Remaining P3: role-routed notifications (§6.3 — dispatch fan-out to service members not just founder userId), lastActiveAt stamping, PARTNER_PREPRESS/PRODUCTION behavioral distinction (currently both = operational access), admin SLA monitor + scorecards, partner payout page.
- 2026-07-02 (Cowork, slice 13): **role-routed fan-out live** — `partnerServiceRecipients` / `partnerOrgAdminRecipients` + `dispatchToPartnerService/Admins` in @ilaunchify/notifications (operational = admins + service members; commercial = admins only; pre-P3 fallback to founder pointer so nothing drops). Migrated: partner-ops-worker (DOC_* → admins; DISPATCH_SLA_AT_RISK, INBOUND_DELIVERED_UNCONFIRMED, RELEASE_SHIP_SLA_AT_RISK → service recipients), receiving-exception resolution, creator proof decisions. **HANDOFF → Code:** routing.ts DISPATCH_RECEIVED sites (3× in packages/orders/src/routing.ts + reprint-dispatch.ts) sit beside frozen findRouting — one-line swaps to dispatchToPartnerService when convenient. Also: lastActiveAt stamped in getPartnerAccess (15-min throttle, fire-and-forget); /payments admin-gated + org-scoped (transfers read the FOUNDER's user row — teammate admins see the org's money) with **Stripe Express dashboard link** (`/api/stripe-express` mints a login link via existing createExpressDashboardLink; never stored). No schema. P3 truly remaining: PREPRESS/PRODUCTION behavioral split, admin SLA monitor + scorecards (needs reprintOfDispatchId col), SlaRule table.
- 2026-07-02 (Cowork, slice 14 — **P3 COMPLETE, plan closed**): `OrderDispatch.reprintOfDispatchId` self-relation added (Code: set it in createReprintDispatch + mechanical backfill from DISPATCH_REPRINT_CREATED payloads); **PartnerScorecard** on admin partner detail (completed dispatches, accept rate, QC failures, discrepancy rate, reprints, avg yield — read-only, warn-tinted, "no automatic consequences" per the tier-binding rule); **SLA monitor** at `/admin/logistics/sla` (acceptance at-risk/breached, receiving unconfirmed nudged/escalated, releases past window — radar only, interventions from the order). Schema: 1 column — db:push + generate. **Deferred with reasons, not forgotten:** PREPRESS/PRODUCTION behavioral split (blocked on Code's print-workflow PRODUCTION_LOCKED gates — roles are captured in memberships, enforcement joins that work); SlaRule table (3 rules don't justify the abstraction — revisit at ~6); blackout/DISPATCH_RECEIVED routing items (findRouting freeze). **Everything else in this plan — P0 through P3 — is BUILT.** Remaining before go-live is operational, not code: FC dry run (docs/FC_DRY_RUN.md), team-invite loop test, print proof-loop walkthrough, Stripe/logistics gate review, Code's routing.ts fan-out swaps + reprint column adoption.
- 2026-07-02 (Cowork, slice 15 — acceptance kit): **docs/GO_LIVE_ACCEPTANCE.md** — the single ~45-min walkthrough that gates real-partner onboarding (supersedes FC_DRY_RUN steps): §A FC loop, §B proof loop w/ reject→v2→approve round trip + READY-gate check, §C team invite/accept/scoped-view/roster/routed-notifications, §D cron idempotency + doc-expiry→digest→suspension chain, §E ramp/scorecard/doc-track/audit, + explicit known-caveats list so expected V1 behavior isn't reported as failure. `seed:fc-dryrun` extended with `demo-order-proof-loop` (LABEL dispatch ACCEPTED on the sample label service — first creator×printer order, so D3 requires the proof). Typecheck fixes from Pavel's run: AuditLog.at (not createdAt) ×3, worker select partnerServiceId.
