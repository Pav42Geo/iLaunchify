...
| Manufacturer references (their own SKUs) | Best practice (EDI/PIM: keep the seller's item id separate from the platform's) — a dedicated **`ProductTemplate.manufacturerRefs` Json** `[{label,value}]` (additive, **needs Mac db push**) for the manufacturer's own tracking codes (ERP id, warehouse code, legacy SKU). Reference-only (platform never keys off it), partner-visibility-only. Importer "Your references" section maps label→column (up to 8); per-product values shown read-only in the preview + persisted on import. **Any sheet column not mapped to an iLaunchify field is offered as a one-click "keep as my reference" chip** (the column header becomes the label) so nothing in the spreadsheet is silently dropped — adding one applies to every imported product (each keeps its own value) and moves it into the references list. References are also **add/edit-able in the builder's Basics step** ("Your references" card, up to 8) and **shown read-only to admins** on the product detail page (sidebar "Manufacturer references" card — reference-only, the platform never routes/matches on them). The **products list has a search box** (name / Base SKU / reference) and surfaces the refs on each card, so a partner finds products by their own code. | `import-actions.ts`, `products/import/ProductImportButton.tsx`, `products/page.tsx`, `products/new/BasicsScreen.tsx`, `products/new/build-actions.ts`, `admin/(dashboard)/products/[id]/page.tsx`, `ProductTemplate` |
