...
| Per-product preview + inline editor | The mapping step's preview is titled by the product name and has **‹ › arrows (“3 / 8”)** to page through every product. Each field is an **editable input** — a manufacturer can tweak any value (name, MOQ, net content…) or fill one the sheet lacked; edits are stored per row (`overrides[rowIndex][field]`) and override the mapped cell at import via `valueFor`. Inputs flag amber ("check") / red ("required") live. | `products/import/ProductImportButton.tsx` |
