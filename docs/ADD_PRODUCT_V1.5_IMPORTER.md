...
| Per-category resolution | When a "Category" column is mapped, a **"Categories in your sheet"** panel lists each distinct source-category value (with a count + a "no match" flag) and a dropdown to send it to an iLaunchify category, **"Use default"**, or **"Skip these products."** Handles a mixed-category sheet (each value filed correctly) and a category iLaunchify doesn't have (taxonomy is LOCKED/admin-curated → map to the closest or skip; partners can't add categories). The "Default category" remains the catch-all for blank/unmapped categories. | `products/import/ProductImportButton.tsx` |
