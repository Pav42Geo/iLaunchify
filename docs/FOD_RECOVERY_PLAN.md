...
> ⚠️ **Model correction 2026-05-19.** During Phase B planning the V1 business model was clarified: iLaunchify is a **B2B production marketplace + manufacturing fulfillment platform**. Consumers do not buy on iLaunchify — they buy on the creator's external channel (Shopify, Amazon, Etsy, WooCommerce, Walmart, TikTok). `apps/storefront` and the consumer-facing models (Cart / CartItem / ConsumerUser) are being removed. See `docs/STOREFRONT.md` for the corrected model and the new "Channel scaffolding + production-order checkout + apps/storefront deletion" chunk that lands before resuming Phase B.

How we get the working FOD operational surface back into iLaunchify, given a "functional parity first, then optimize" approach. This plan covers **Partner-facing surfaces** and the **Admin dashboard**. Creator marketplace + customize flow continues on the existing roadmap; the previously-planned hosted storefront has been retired and replaced with a much smaller "channel publish" surface (V1.1+).
...
| B3 | **Admin product moderation** — list + approve/reject actions on creator-published products, with bulk status, revert, and inventory adjust | M | Once creators start publishing finished SKUs (pushing to their channels), admins need an override path for compliance escalations and recalls before the SKU leaves iLaunchify. |
...
| C3 | ~~**Theme system** (admin theme registry + storefront theme editor)~~ | — | Removed 2026-05-19. iLaunchify no longer hosts consumer storefronts (per model correction), so a theme system has no surface to apply to. The `Brand` model retains color/font metadata for label rendering + future channel-listing customization. |
