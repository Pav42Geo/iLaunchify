...
- [x] Checkout surfacing (PS-3b, 2026-07-06): Order Summary "Printed by X — your pick" annotation under the Label-printing line (server-loaded in page.tsx, display-only); in-checkout picker modal deferred — the pick lives on the marketplace card **[CW]**
- [x] `findRouting` step 0: pinned printer validated against the same hard ops gate (ACTIVE svc/partner/Stripe, ≥1 ACTIVE offering, no live blackout, not excluded) → binds exactly; otherwise falls through to auto-routing with `pinnedPrintUnavailable` flag on the result — the pin is a preference, never a filter bypass (§4). Checkout wired via cart-actions lookup **[CW, routing.ts handoff with CODE]**
- [x] Pinned-reroute approval gate (PS-3b, 2026-07-06): `pinnedPrintUnavailable` → placeOrder declines with `pinnedPrintGate` (both provider names); `PinnedPrintGatePanel` modal pre-payment — "Continue with [auto]" (ack, audited ORDER_PINNED_PRINT_REROUTE_ACKED) or "Keep my pick — don't order yet". Mirrors the capacity-gate pattern; a manual pick is never silently rerouted **[CW]**
- [ ] Studio print-spec pinned-provider indication **[CODE — canvas hot zone]**
- [x] FC "Can finalize labeling here" (PS-3c, 2026-07-06): `labeling-actions.ts` shared `computeFcLabelingContext` (badge + charge derive from ONE function — applied decoration + external print + mfr !appliesLabels + verified ACTIVE RELABEL VAS matching method); badge on suggested-FC line + warehouse picker cards; explicit opt-in checkbox w/ fee-per-unit + min-units + lead-time copy; "FC labeling × units" line in OrderSummary; placeOrder re-derives (min-units hard check, stale flag = fee silently dropped, default mfr-finalizes stands), fee joins subtotal + platform-fee base, opt-in recorded in internalNotes + ORDER_CREATED payload (§8.1a) **[CW]**
- [x] Per-hop shipping (PS-3d, 2026-07-06 — Pavel decided: printer→applier hop freight bills to the creator's SHIPPING line, ONE line + expandable breakdown): `shipping-hops.ts` pure module (`estimateLabelHopCents` V1 flat rate-card: $12 base + $3/500 units; `labelHopCopy` mfr vs FC); `computeFcLabelingContext` gained `externalLabelHop` (true for ANY external print — applied AND printed-in methods; decorated packaging must reach the producer either way; IN_HOUSE = no hop); `estimateShipping` folds the hop into every path (carrier, flat, even HOLD_AT_MANUFACTURER — goods leg 0, label hop still real) + returns `hops[]`; OrderSummary "N legs — see breakdown" expandable under the single Shipping row; placeOrder books combined figure (free-shipping threshold covers the total), hop split recorded in internalNotes **[CW]**
- [ ] Publish + checkout graph pre-flight gates (§8 UNRESOLVED validator at publish + Pay) **[CW; needs PAVEL: UNRESOLVED checkout fallback order]**
