...
| L6 | Infant formula | **Excluded platform-wide at the ENTRY POINT, not at checkout** (Pavel 2026-07-02: never let a creator do the work and then block shipping). Infant formula = breast-milk substitute under 21 CFR 106/107 — NOT baby food (purees/snacks stay fully supported). Gates: (1) New Product Step 1 — format/category not selectable, server-enforced DomainSetting-style (OTC pattern); (2) marketplace — manufacturers cannot publish infant-formula ProductTemplates, admin declines at review; (3) checkout/routing block remains ONLY as defense-in-depth for legacy/edge data and should never fire. |
